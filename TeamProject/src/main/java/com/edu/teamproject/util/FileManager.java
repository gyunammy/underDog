...
public class FileManager {
	...
	public void saveproduct(Product product, String dir) throws UploadException{
		...
		MultipartFile[] productPList=product.getProductPhoto();
		...
		try {
			for(MultipartFile photo : productPList) {
				...
				productImg.setFilename(createFileName(photo.getOriginalFilename()));
				
				//리스트에 productImg 추가
				productImgList.add(productImg);
				
				//파일생성?
				photo.transferTo(new File(dir+productImg.getFilename()));
			}
